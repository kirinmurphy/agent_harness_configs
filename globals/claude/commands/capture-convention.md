...
Global harness items (caveman behavior, jcodemunch defaults, verification discipline, hook patterns) go into `globals/claude/CLAUDE.md` or a new doc under `docs/`. Do not invent new global rules — only promote items explicitly confirmed as cross-repo behavior.

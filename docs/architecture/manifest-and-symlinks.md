...
`globals/manifest.tsv` — is the **one list** every script reads to know what to link, verify,
or clean up.
...
| Term | Meaning |
| --- | --- |
| **manifest** | `globals/manifest.tsv` — the single list of managed home↔repo paths. An inventory: *these things are managed, and how.* |
| **source-files list** | `globals/source-files.tsv` — separate checklist of repo files that must exist (asserted by doctor). Tracks repo health, not home symlinks. |
| **Bash reader** | `scripts/lib/globals-data.sh` — bash funcs (`manifest_rows`, `source_files`) that parse the data files so POSIX scripts do not hardcode the list. |
| **PowerShell reader** | `scripts/install/install-windows.ps1` — parses `globals/manifest.tsv` directly for Windows installs. |
...
flowchart LR
  ...
  subgraph manifest["globals/manifest.tsv"]
...
```mermaid
flowchart TD
  tsv["globals/manifest.tsv<br/>(single source of truth)"]
  reader["scripts/lib/globals-data.sh<br/>manifest_rows()"]
  tsv --> reader

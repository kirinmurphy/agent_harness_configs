...
# Manifest guard: every link/root_config row in manifests/manifest.tsv must name a real repo
...
check_manifest_sources() {
  ...
  while IFS=$'\t' read -r _h kind src_rel _home _flags; do
    ...
    if [[ ! -e "${repo_root}/${src_rel}" ]]; then
      fail "manifest source missing: ${src_rel} (referenced by manifests/manifest.tsv)"
      bad=1
    fi
  done < <(manifest_rows)
...
if [[ "${check_installed}" -eq 1 ]]; then
  ...
  # Managed link + root-config checks come from manifests/manifest.tsv. Rows flagged "nodoctor"
